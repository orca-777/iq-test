 ...
 * Turso 云端数据库初始化脚本
 *
 * 用途：在 Turso 云端创建表结构并导入种子数据
 * 前提：已完成以下步骤
 *   1. 在 https://turso.tech 注册账号
 *   2. 创建数据库（如 assessment）
 *   3. 创建 Auth Token
 *
 * 使用方法：
 *   set TURSO_DATABASE_URL=libsql://assessment-xxx.turso.io
 *   set TURSO_AUTH_TOKEN=xxx
...
if (!url || !token) {
  ...
  console.error('   set TURSO_DATABASE_URL=libsql://your-db-name.turso.io');
  console.error('   set TURSO_AUTH_TOKEN=your-auth-token');
  process.exit(1);
}

async function run() {
  console.log(`🔗 连接 Turso: ${url.replace(/libsql:\/\/[^-]+-/,'libsql://***-')}`);
  ...
  console.log('✅ 4 张表创建完成');

  // ── 默认配置 ──────────────────────────────────────────
  console.log('\n⚙️  写入默认配置...');
  const settings = [
    { key: 'shuffle_questions', value: 'true' },
    { key: 'exam_duration_minutes', value: '60' },
  ];
  ...
  // ── 管理员账号 ──────────────────────────────────────────
  console.log('\n👤 创建管理员账号...');
  const existing = await client.execute('SELECT COUNT(*) as cnt FROM admin_users');
  const userCount = Number(existing.rows[0][0]);
  ...
  if (userCount === 0) {
    ...
    const users = [
      { username: 'admin', hash: adminHash, role: 'super', name: '超级管理员' },
      { username: 'examadmin', hash: examHash, role: 'exam_admin', name: '考务管理员' },
      { username: 'viewer', hash: readHash, role: 'readonly', name: '只读用户' },
    ];
    for (const u of users) {
      await client.execute({
        sql: `INSERT INTO admin_users (username, password_hash, role, display_name) VALUES (?, ?, ?, ?)`,
        args: [u.username, u.hash, u.role, u.name],
      });
    }
    console.log('✅ 3 个管理员账号已创建');
  } else {
    console.log(`ℹ️  已有 ${userCount} 个账号，跳过`);
  }

  // ── 题库 ──────────────────────────────────────────────
  console.log('\n📚 导入题库...');
  const existingQ = await client.execute('SELECT COUNT(*) as cnt FROM questions');
  const qCount = Number(existingQ.rows[0][0]);
  ...
  if (qCount === 0) {
    ...
    for (let i = 0; i < questions.length; i++) {
      const q = questions[i];
      await client.execute({
        sql: `INSERT INTO questions (id, part, dimension, order_num, q_text, options, answer, q_type)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [i + 1, q.part, q.dimension, q.order_num, q.q_text,
               JSON.stringify(q.options), q.answer, q.q_type],
      });
    }
    console.log(`✅ ${questions.length} 题导入完成`);
  } else {
    console.log(`ℹ️  已有 ${qCount} 题，跳过导入`);
  }

  // ── 验证 ──────────────────────────────────────────────
  console.log('\n🔍 验证数据...');
  const tables = [
    { name: 'questions', sql: 'SELECT COUNT(*) FROM questions' },
    { name: 'admin_users', sql: 'SELECT COUNT(*) FROM admin_users' },
    { name: 'app_settings', sql: 'SELECT COUNT(*) FROM app_settings' },
    { name: 'assessment_results', sql: 'SELECT COUNT(*) FROM assessment_results' },
  ];
  for (const t of tables) {
    const r = await client.execute(t.sql);
    console.log(`   ${t.name}: ${r.rows[0][0]} 行`);
  }

  console.log('\n🎉 Turso 云端数据库初始化完成！');
  console.log('\n📌 下一步：在 Vercel 控制台配置环境变量');
  console.log(`   TURSO_DATABASE_URL = ${url}`);
  console.log(`   TURSO_AUTH_TOKEN = ${token.replace(/./g, '*')}`);
  console.log('\n   Vercel 项目 → Settings → Environment Variables');
  console.log('   添加以上两个变量（Production / Preview / Development 全勾选）');
}

// ── 完整题库数据（与 init-db.js 保持一致）──────────────────
